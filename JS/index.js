...
const crearTarjetaCancion = (cancion) => {
  const tarjetaHTML = `
    <div class="card tarjetaCancion p-3 mb-2" id="${cancion.id}">
      <div class="d-flex flex-column position-relative">
        <img src="${cancion.imagen}" class="card-img-top rounded-2" alt="" />
      </div>
      <div class="card-body ps-0 pb-0">
        <h6 class="card-title">${cancion.titulo}</h6>
...
const crearSeccion = (categoria) => {
  ...
  seccion.classList.add('mt-5','ps-3','ps-sm-0','px-0');

  const h2 = document.createElement('h2');
  h2.classList.add('pb-4', 'ms-3', 'ms-md-0');
